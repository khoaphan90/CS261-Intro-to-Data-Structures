...
*
* Overview:
* Practice with heaps using dynamic arrays
*
* Input:
* No user input.
*
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	// get positions of children from parent
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
    ...
    int position = dySize(heap) - 1;
    int parent = (position-1) / 2;

	void* parentValue = heap->data[parent];

	while (position > 0 && compare(value, parentValue) == -1)
	{
		dySwap(heap, position, parent);
		position = parent;
		parent = (position - 1) / 2;
		parentValue = heap->data[parent];
	}
}
